...
def update
  if @task.update(task_params)
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.replace("edit_task", partial: "tasks/edit_form", locals: { task: @task }), status: :unprocessable_entity
      end
...
def destroy
  ...
  def task_params
    params.require(:task).permit(:title, :category_id, :deadline, :priority_id, :details, :status_id, :needs_editing).merge(user_id: current_user.id)
  end
